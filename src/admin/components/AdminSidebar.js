...
const AdminSidebar = () => {
  ...
  // États pour les animations live
  ...
  // 🔄 Chargement des compteurs + animation live
const fetchAllCounts = async () => {
  ...
  // 🔔 Déclenche une animation “live update”
  const triggerLiveChange = (field, delta) => {
    if (delta === 0) return;
    setLiveChange({ field, delta });

    // Cache l'animation après 1.5s
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-card-dark shadow-lg min-h-screen p-5 relative overflow-hidden">
      {/* 🧭 En-tête */}
      <div className="text-2xl font-bold text-primary mb-10 tracking-tight flex items-center justify-between">
        🚍 Kocrou Admin
      </div>
      ...
      <nav className="flex-1 space-y-2">
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {liveChange.field === item.key && liveChange.delta !== 0 && (
                    ...
                    >
                      {liveChange.delta > 0 ? `+${liveChange.delta}` : liveChange.delta}
                    </motion.span>
                  )}
                </AnimatePresence>
              </div>
        ...
        {/* 🟢 Monitoring Live avec animation */}
        <NavLink
          to="/admin/live-monitor"
          className={({ isActive }) => `flex items-center justify-between px-4 py-2 rounded-lg font-medium transition ${isActive? 
          "bg-primary text-white": "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"}` } >
          <div className="flex items-center gap-2">
            <Activity className={`w-4 h-4 ${adminCount > 0 ? "text-green-500 animate-pulse" : "text-gray-400" }`} />
      ...
      <div className="mt-auto border-t border-gray-200 dark:border-gray-700 pt-4">
        {/* Indicateur de connectivité WebSocket */}
